  ...
  try:
    ...
    try:
      ...
      if os.path.exists(img_path_jpg):
        ...
        st.info(
            f"🖼️ 圖片佔位區：若要顯示圖片，請將圖片命名為 `picture_{idx}.jpg` 或"
            " `.png`，並放置於 `assets/images/` 資料夾中。"
        )
    except:
      pass
...
def render_section(section_name, db):
  ...
  if not questions:
    st.warning(
        f"⚠️ 系統抓不到【{section_name}】的資料。請確認題庫 TXT 檔案已上傳至"
        " GitHub。"
    )
    return
...
def main():
  ...
  st.markdown(
    ...
    /* 1. 全域背景：柔和海洋水藍色漸層 */
    .stApp {
        background: linear-gradient(180deg, #E0F2FE 0%, #F0F9FF 60%, #F8FAFC 100%) !important;
  ...
  # 🌊 頂部 Banner 封面 (若在 assets/banner.png 有圖片則自動呈現)
  banner_path = "assets/banner.png"
  if os.path.exists(banner_path):
    st.image(banner_path, use_container_width=True)

  st.title("🌊 阿美語中高級認證練習平台")
  st.caption("Paso'elinay to sowal no Pangcah — 請選擇練習題型開始學習")

  main_options = ["📋 認證考試說明", "🎧 聽力", "🗣️ 口說", "📖 閱讀", "✍️ 寫作"]
  current_tab = st.segmented_control(
      "主選單導覽", main_options, default=None, label_visibility="collapsed"
  )
  ...
  if current_tab == "📋 認證考試說明":
    st.subheader("📋 認證考試說明 (Pasi'ocoran)")
    st.divider()
    st.info(
        "歡迎使用本平台！請透過上方導覽列選擇您要練習的測驗項目（聽力、口說、閱讀、寫作）。系統將自動解析載入題庫。"
    )
